...
const VoteHelp = ({ open, close }) => {
  return (
    <div className="fixed top-0 left-0 right-0 bottom-0 z-30 bg-black/70">
      <div
        className={`help z-40 shadow-lg transition duration-1000 ease-linear  fixed top-0 right-0 bottom-0 bg-gray-50 w-9/12 md:w-1/2 lg:w-4/12 p-2 translate-x-full ${
          open ? "translate-x-0" : ""
        }`}
      >
        <div className="mb-5">
          <button
            onClick={close}
            className="py-2 px-4 border-2 border-gray-400"
          >
            <FaTimes size={20} />
          </button>
        </div>
